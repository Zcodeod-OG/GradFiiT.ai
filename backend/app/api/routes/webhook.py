"""
GradFiT - Webhook Handlers
Handle webhooks from external services (Replicate, Stripe, Fashn, etc.)

Clerk has been removed -- GradFiT consolidates auth on the JWT issued by
``app.api.routes.auth``. Frontend signups go through /api/auth/register
and the legacy /api/webhooks/clerk surface is gone.
"""
